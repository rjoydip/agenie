...
export const PlanTaskSchema = z.object({
  ...
  taskType: z
    .enum(["use_tools", "reason"])
    .describe(
      "use_tools = needs tools to fetch data, reason = LLM analysis only",
    ),
  dependsOn: z
    .array(z.string())
...
export const PlanSchema = z.object({
  summary: z.string().describe("One sentence summary under 15 words"),
  tasks: z.array(PlanTaskSchema).describe("2-5 tasks with short descriptions"),
});
...
export const ReflectionSchema = z.object({
  isComplete: z
    .boolean()
    .describe(
      "Whether we have sufficient information to answer the query fully",
    ),
  reasoning: z
    .string()
  ...
  missingInfo: z
    .array(z.string())
    .describe(
      "Specific pieces of information still needed (empty array if complete)",
    ),
  suggestedNextSteps: z
    .string()
    .describe(
      "High-level guidance for what to do next (empty string if complete)",
    ),
});
