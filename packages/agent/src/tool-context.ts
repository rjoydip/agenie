...
export class ToolContextManager {
  ...
  constructor(contextDir: string = CONTEXT_DIR, model: string = DEFAULT_MODEL) {
  ...
  saveAndGetSummary(
    ...
    const filepath = this.saveContext(
      toolName,
      args,
      result,
      undefined,
  ...
  async selectRelevantContexts(
    ...
    try {
      const response = await callLlm(prompt, {
        ...
        model: this.model,
        schema: SelectedContextsSchema,
      });

      const selectedIds =
        (response as { context_ids: number[] }).context_ids || [];
      ...
      console.warn(
        `Warning: Context selection failed: ${e}, loading all contexts`,
      );
