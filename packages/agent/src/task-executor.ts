...
export interface TaskExecutorCallbacks extends ToolExecutorCallbacks {
  ...
  onTaskToolCallsSet?: (taskId: string, toolCalls: ToolCalls[]) => void;
}
...
export class TaskExecutor {
  ...
  private async executeTask(
    ...
    if (task.taskType === "use_tools") {
      const toolCalls = await this.toolExecutor.selectTools(
        task,
        understanding,
      );
      ...
      if (toolCalls.length > 0) {
        toolsSucceeded = await this.toolExecutor.executeTools(
          task,
          queryId,
          callbacks,
        );
  ...
  private buildContextData(
    ...
    if (pointers.length > 0) {
      const contexts = this.contextManager.loadContexts(
        pointers.map((p) => p.filepath),
      );
      ...
      for (const ctx of contexts) {
        ...
        parts.push(
          `Data from ${toolName} (${JSON.stringify(args)}):${sourceLine}\n${JSON.stringify(result, null, 2)}`,
        );
      }
    }
