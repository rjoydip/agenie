...
import { AgentProgressState } from "../types";
...
export function useAgentExecution({
  ...
  const [answerStream, setAnswerStream] =
    useState<AsyncGenerator<string> | null>(null);
   ...
   */
  const setTaskToolCalls = useCallback(
    (taskId: string, toolCalls: ToolCalls[]) => {
      setCurrentTurn((prev) => {
        if (!prev) return prev;
  ...
  const updateToolCallStatus = useCallback(
    (taskId: string, toolIndex: number, status: ToolCalls["status"]) => {
      setCurrentTurn((prev) => {
        if (!prev) return prev;
        ...
        const tasks = prev.state.tasks.map((task) => {
          ...
          const toolCalls = task.toolCalls.map((tc: any, i: number) =>
  ...
  const handleToolCallError = useCallback(
    (
      ...
      setToolErrors((prev) => [
        ...prev,
        { taskId, toolName, args, error: error.message },
      ]);
    },
    [],
  );
