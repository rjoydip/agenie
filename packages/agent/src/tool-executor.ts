...
export class ToolExecutor {
  ...
  private readonly toolMap: Map<string, ToolSet[keyof ToolSet]>;
  private readonly contextManager: ToolContextManager;

  constructor(options: ToolExecutorOptions) {
    this.tools = options.tools;
    this.toolMap = new Map();
    for (const key of Object.keys(this.tools) as Array<keyof ToolSet>) {
      this.toolMap.set(key, this.tools[key]);
    }
  ...
  private formatToolDescriptions(): string {
    return Object.values(this.tools)
      .map((tool) => {
        ...
        return `- ${tool.title}: ${tool.description}${argsDescription}`;
      })
      .join("\n\n");
  }
  ...
  private extractToolCalls(
    ...
    if (!message.content.tool_call || !Array.isArray(message.content.tool_call))
      return [];

    return message.content.tool_call.map((tc) => ({
      tool: tc.name,
